resource "aws_autoscaling_policy" "high_cpu" {
  ...
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.main_app_instance_asg.name 
}
...
resource "aws_cloudwatch_metric_alarm" "app1_asg_cwa_cpu" {
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.main_app_instance_asg.name 
  }
  ...
  ok_actions          = [aws_sns_topic.ASG_notification.arn]  
  alarm_actions     = [
    aws_autoscaling_policy.high_cpu.arn, 
    aws_sns_topic.ASG_notification.arn
    ]
}